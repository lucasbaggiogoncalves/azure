try {                
    ...
    Write-Output "Autenticação efetuada."
}
catch {
    throw "Falha na autenticação.`n$($_.Exception)"
} 
    ...
    Begin {
        ...
        If ($Filter) {
            If ($Filter.ToUpper().IndexOf($Property.ToUpper()) -ge 0) {
                ...
                Catch {
                    Write-Warning "$(Get-Date): ""$Filter"" erro, parando script!"
                    Write-Warning $Error[0]
                    Exit
                }
            }
            Else {
                Write-Warning "Não foi possível localizar $Property no filtro, é obrigatório.  Filtro: $Filter"
                Exit
            }
        }
    }
    ...
    Process {
        ForEach ($Line in $InputObject) {
            If ($Line.IndexOf("<tr><th") -ge 0) {
                Write-Verbose "$(Get-Date): Processando headers..."
